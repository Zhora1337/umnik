...
def run_frank_mocap(args, bbox_detector, body_mocap, hand_mocap, visualizer):
    ...
    while True:
        ...
        if hand_bbox_list[0]['right_hand'] is not None and hand_bbox_list[0]['left_hand'] is not None:
            if (((hand_bbox_list[0]['right_hand'][0] + hand_bbox_list[0]['right_hand'][0] +  hand_bbox_list[0]['right_hand'][2]) / 2)  < body_bbox_list[0][0] and
            ((hand_bbox_list[0]['left_hand'][0] + hand_bbox_list[0]['left_hand'][0] + hand_bbox_list[0]['left_hand'][2]) / 2)  > (body_bbox_list[0][0] + body_bbox_list[0][2])):
                right_hand_text = 'right_hand in right'
                left_hand_text = 'left_hand in left'
                up_frames += 1
            else:
                up_frames = 0
 ...
 #     print('left hand ', hand_bbox_list[0]['left_hand'])
  #      print('body ' , body_bbox_list)
        ...
        res_img = visualizer.visualize(
            ...
            frame_text1=right_hand_text,
            frame_text2=left_hand_text)

        cv2.putText(res_img, f'Count: {count}', (15,40), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 0, 255), 1)
