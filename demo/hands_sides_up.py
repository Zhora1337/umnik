...
def run_frank_mocap(args, bbox_detector, body_mocap, hand_mocap, visualizer):
    ...
    while True:
        ...
        body_bbox_list, hand_bbox_list = run_regress(
            args, img_original_bgr, 
            body_bbox_list, hand_bbox_list, bbox_detector,
            body_mocap, hand_mocap)
        ...
        if hand_bbox_list[0]['right_hand'] is not None and hand_bbox_list[0]['left_hand'] is not None:
            if (hand_bbox_list[0]['right_hand'][1] < body_bbox_list[0][1]) and (hand_bbox_list[0]['left_hand'][1] < body_bbox_list[0][1]):
                right_hand_text = 'right_hand above head'
                left_hand_text = 'left_hand above head'
                up_frames += 1
            else:
                up_frames = 0
       ...
       # show result in the screen
        ...
        if args.save_pred_pkl:
            ...
            demo_utils.save_pred_to_pkl(
                args, demo_type, image_path, body_bbox_list, hand_bbox_list)
