...
def draw_bbox(image, bbox, color=(0,0,255), text = '', thickness=3):
    ...
    cv2.putText(res_img, text, (x0,y1), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 255, 255), 1)
...
def draw_hand_bbox(img, hand_bbox_list, frame_text1, frame_text2):
    img = img.copy()
        ...
        if hand_bboxes is not None:
            for key in hand_bboxes:
                ...
                if bbox is not None:
                    ...
                    if frame_text1.__contains__(key):
                        img = draw_bbox(img, bbox_new, color=color, text=frame_text1)
                    elif frame_text2.__contains__(key):
                        img = draw_bbox(img, bbox_new, color=color, text=frame_text2)
                    else:
